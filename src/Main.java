...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < products.length; i++) {
            if (counts[i] != 0) {
                int currentCount = (doBonus ? counts[i] + 1 : counts[i]);
                boolean isOnSale = false;
                for (String saleProduct : productsOnSale) {
                    if (products[i].equals(saleProduct)) {
                        isOnSale = true;
                ...
                if (isOnSale) {
                    System.out.println("\t" + products[i] + " " + currentCount + " шт. за " + (prices[i] * (counts[i] / 3 * 2 + counts[i] % 3)) + " руб. (распродажа!)");
                } else {
                    System.out.println("\t" + products[i] + " " + currentCount + " шт. за " + (prices[i] * counts[i]) + " руб.");
                }
            }
        }
